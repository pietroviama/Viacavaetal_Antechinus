...
map2 <- 
  ...
  scale_fill_manual(values = c("grey", "red", "blue", "hotpink", "darkgreen", "orange"), labels=c("?", "stuartii north", "stuartii south", "stuartii neotype", "subtropicus", "subtropicus holotype"), name=NULL) +
  ...
  theme_bw() + theme(legend.position="bottom", axis.text = element_text(size = 15), text = element_text(size = 15), legend.key = element_rect(colour = "white"), axis.text.x = element_text(angle=45, vjust=0.5), legend.text = element_text(size = 15)) # tweak the plot's appearance and legend position
