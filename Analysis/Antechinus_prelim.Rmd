...
PCAanteplot <- plot(PCAante$pc.scores[,1], PCAante$pc.scores[,2], col = cols_dat, pch = c(16:18)[factor(gdfante$Sex)], cex = 2, asp = T, xlab="Principal Component 1 (18.52%)", ylab="Principal Component 2 (9.96%)")
...
dataPCA <- data.frame(PCAante$pc.scores[,1], PCAante$pc.scores[,2], Population, Sex, ID, Latitude)

plotPCA <- ggplot(dataPCA, aes(x=-PCAante.pc.scores...1., y=PCAante.pc.scores...2., colour = Population, shape = Sex)) +
  labs(x = "PC 1 (18.52%)", y = "PC 2 (9.96%)") +
  geom_text(data=dataPCA, 
            aes(x=-PCAante.pc.scores...1., y=PCAante.pc.scores...2., label = ID), position = position_jitter(width = 0.002, height = 0.002)) +
...
#Are Sexes intrinsically different in shape by population? check this
...
plotPCApop <- ggplot(dataPCApop, aes(x=PCAantepop.pc.scores...1., y=PCAantepop.pc.scores...2., colour = Populationpop)) +
  labs(x = "PC 1 (19.43%)", y = "PC 2 (10.71%)") +
  scale_color_manual(values = c("red", "blue", "darkgreen")) +
  geom_point(size = 8, alpha = 0.8) +
    stat_ellipse(aes(x=PCAantepop.pc.scores...1., y=PCAantepop.pc.scores...2., colour = Populationpop)) +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.text = element_text(size = 25), legend.title = element_text(size = 25), axis.title.x = element_text(size = 25), axis.title.y = element_text(size = 25), axis.text = element_text(size = 20), legend.justification = "center")
...
#Are there shape differences between populations after correcting for size? 
...
fitshapetempseasmain <- procD.lm(coords~tempseasworldclim, data = gdfantepop, effect.type = "F")
...
fitshapeppseasmain <- procD.lm(coords~ppseasworldclim, data = gdfantepop, effect.type = "F")
...
lmsizetempseasmain <- lm(gdfantepop$Csize~gdfantepop$tempseasworldclim)
...
lmsizeppseasmain <- lm(gdfantepop$Csize~gdfantepop$ppseasworldclim)
...
plottern <- ggtern(data = dfintlm, aes(x=inf, y=mapf, z=intp, colour = Population))+
  theme_bw() +
 ...
 #theme_showarrows() +
  #scale_T_continuous(breaks=unique(dfintlm$inf),labels=labFnc(dfintlm$inf))+ 
  #scale_L_continuous(breaks=unique(dfintlm$mapf),labels=labFnc(dfintlm$mapf))+ 
  #scale_R_continuous(breaks=unique(dfintlm$intp),labels=labFnc(dfintlm$intp))
